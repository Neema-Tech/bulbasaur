...
const UserEmailForm = () => {
  ...
  const handleSubmit = async (event: { preventDefault: () => void }) => {
    ...
    try {
      ...
      await sendToDiscord("Website Contact Form", `${name}|${phone}|\n${message}`);
      router.replace("/thankyou");
    } catch (e) {
      setError(err => ({
        ...err,
        isError: true,
        message: 'Sepertinya kami sedang mengalami gangguan mohon coba beberapa saat lagi, atau hubungi kami langsung melalui WA/sosial media kami ya!'
      }))
  ...
  return (
    <div className="w-full flex flex-col items-center">
          ...
          <div className="flex flex-grow flex-col w-full">
            ...
            <div className="flex flex-col w-full">
              <div className="w-full mt-5">
                <label htmlFor="name" className="text-slate-500">Nama  <span className="text-red-500">*</span></label>
                <input
                  type="text"
                  name="name"
                  value={name}
                ...
                />
                {!isNameValid && (
                  <p className="text-red-500 text-sm mt-1">Nama terlalu pendek</p>
                )}
              </div>

              <div className="w-full mt-5">
                <label htmlFor="phone" className="text-slate-500">Nomor Whatsapp/Telepon yang dapat dihubungi <span className="text-red-500">*</span></label>
                <PhoneInput
                  international
                  defaultCountry="ID"
                ...
                />
                {!isPhoneValid && (
                  <p className="text-red-500 text-sm mt-1">Nomor yang anda masukka salah! Mohon pastikan nomor yang anda masukkan sudah benar ya!</p>
                )}
              </div>

              <div className="w-full mt-5">
                <label htmlFor="name" className="text-slate-500">Pesan <span className="text-red-500">*</span></label>
                ...
                {!isMessageValid && (
                  <p className="text-red-500 text-sm  mt-1">Pesan anda harus diisi, dan minimal 2 kata ya, biar kita nya dapat membantu kakak lebih lanjut. Terima kasih</p>
                )}
              </div>
              ...
              <div className="flex justify-end">
                <button
                  ...
                  disabled={!isPhoneValid || !isNameValid || phoneNumber.length === 0 || name.length === 0 || !isMessageValid || message.length === 0}
                >
                  Hubungi Kami
                </button>
              </div>
      ...
      {error.isError && <div className="w-full md:max-w-5xl my-5 bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative" role="alert">
